...
public class AdminDelWarpCommand implements SubCommand, AsyncTabComplete {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        api.getPlayerUuid(warpPlayerName).thenCompose(targetUuidOpt -> {
            ...
            return api.delWarp(new Actor.Bypass(sender.getName()), targetUuid, warpName).thenRun(() -> sender.sendMessage(config.getAdminDelWarpSuccessMessage(warpPlayerName, warpName)));
        }).exceptionally(ex -> {
    ...
    public CompletableFuture<List<String>> tabCompleteAsync(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 3) {
            ...
            return api.getPlayerUuid(args[1]).thenCompose(uuidOpt -> {
                ...
                return api.getWarpNames(targetUuid).thenApply(names -> names.stream().filter(name -> name.toLowerCase(Locale.ROOT).startsWith(prefix)).sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList()));
