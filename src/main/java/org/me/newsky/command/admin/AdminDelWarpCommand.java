...
public class AdminDelWarpCommand implements SubCommand, AsyncTabComplete {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        api.getPlayerUuid(warpPlayerName).thenCompose(targetUuidOpt -> {
            ...
            return api.getIslandUuid(targetUuid).thenCompose(islandUuid -> api.admin(sender).deleteWarp(islandUuid, warpName)).thenRun(() -> sender.sendMessage(config.getAdminDelWarpSuccessMessage(warpPlayerName, warpName)));
        }).exceptionally(ex -> {
    ...
    public CompletableFuture<List<String>> tabCompleteAsync(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 3) {
            ...
            return api.getPlayerUuid(args[1]).thenCompose(uuidOpt -> {
                ...
                return api.getIslandUuid(targetUuid).thenCompose(api::getWarpNames).thenApply(names -> names.stream().filter(name -> name.toLowerCase(Locale.ROOT).startsWith(prefix)).sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList()));
