...
public class AdminAddMemberCommand extends BaseAddMemberCommand {
    ...
    public AdminAddMemberCommand(ConfigHandler config, CacheHandler cacheHandler, IslandHandler islandHandler) {
    ...
    protected boolean validateArgs(CommandSender sender, String[] args) {
        if (args.length < 3) {
            sender.sendMessage(config.getAdminAddMemberCommandUsage());
            return false;
        }
        return true;
    }
